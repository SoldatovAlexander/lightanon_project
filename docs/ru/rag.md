...
Для интеграций, где важна роль организации, передавайте объекты `OrganizationProfile` в `TextSanitizer`. Тогда `company` скрывает только известные значения собственной компании, а `company_and_counterparties` также скрывает известных контрагентов. Задайте `unknown_organization_policy`: `report`, `mask` или `reject`; общий режим без профиля остаётся для совместимости, но не может определить роль организации только по тексту.
